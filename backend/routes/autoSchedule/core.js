...
export async function diagnoseGroupFailure({
  ...
  const hasPreferredTimeWindow = preferredTimeWindows.length > 0;
...
export async function scheduleGroup({
  ...
  const preferredTimeWindows = normalizePreferredWindows(group?.preferred_time_windows);
  const hasPreferredTimeWindow = preferredTimeWindows.length > 0;
  const useLockedRecurringSlots = !hasPreferredTimeWindow;
  const blockedDateSet = normalizeBlockedDateSet(blockedDates);

  if (useLockedRecurringSlots) {
  ...
  for (const weekStart of weekStarts) {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      for (let day = new Date(weekStartBound); day <= weekEndBound; day.setDate(day.getDate() + 1)) {
        ...
        const enforcePreferred = hasPreferredTimeWindow;
        for (const slot of dayAvailability) {
          ...
          for (let candidate = slotStartMin; candidate + durationMinutes <= slotEndMin; candidate += 30) {
            ...
            const uncoveredTypeIds = typeIds.filter(
              (typeId) =>
            ...
            if (useLockedRecurringSlots && lockedSlots.length === 0) {
  ...
  if (groupScheduled.length === 0) {
    ...
    if (autoSuggestionDepth < 2 && Array.isArray(suggestions) && suggestions.length > 0) {
      for (const suggestion of suggestions) {
        if (hasPreferredTimeWindow && suggestion?.type === "timeslot") continue;
